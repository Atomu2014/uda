...
def main(_):
  ...
  if FLAGS.do_train:
    tf.logging.error("  >>> sup data dir : {}".format(FLAGS.sup_train_data_dir))
    if FLAGS.unsup_ratio > 0:
      tf.logging.error("  >>> unsup data dir : {}".format(
  ...
  if FLAGS.do_eval:
    tf.logging.error("  >>> dev data dir : {}".format(FLAGS.eval_data_dir))
  ...
  if FLAGS.do_train and FLAGS.do_eval:
    tf.logging.error("***** Running training & evaluation *****")
    tf.logging.error("  Supervised batch size = %d", FLAGS.train_batch_size)
    tf.logging.error("  Unsupervised batch size = %d",
                    FLAGS.train_batch_size * FLAGS.unsup_ratio)
    tf.logging.error("  Num steps = %d", FLAGS.num_train_steps)
    tf.logging.error("  Base evaluation batch size = %d", FLAGS.eval_batch_size)
    tf.logging.error("  Num steps = %d", eval_steps)
    best_acc = 0
    ...
    for ckpt_path in checkpoint_state.all_model_checkpoint_paths:
      if not tf.gfile.Exists(ckpt_path + ".data-00000-of-00001"):
        ...
        continue
      tf.logging.error("Evaluating {:s}".format(ckpt_path))
